#!/usr/bin/env python3
"""
音频格式和质量控制功能测试模块 (快速版本)
Tests audio format and quality control functionality (fast version)
只测试WAV和MP3格式，避免耗时的FLAC无损压缩测试
"""
...
class TestAudioFormatQuality(unittest.TestCase):
    """音频格式和质量控制测试类 (只测试WAV和MP3)"""
    ...
    def test_get_supported_formats(self):
        ...
        # 注意：仍然验证FLAC在API中存在，但不进行耗时的分离测试
...
def run_format_quality_tests():
    """运行音频格式和质量测试的函数入口"""
    print("🎵 音频格式和质量控制功能测试 (快速版本)")
    print("📝 注意：为提高测试速度，已跳过耗时的FLAC无损压缩测试")
    print("="*50)
    
    # 创建测试套件
    suite = unittest.TestLoader().loadTestsFromTestCase(TestAudioFormatQuality)
    ...
    print(f"   测试格式: WAV, MP3 (跳过FLAC无损测试)")
